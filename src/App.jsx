...
function generatePDFHTML(d, client, period) {
  ...
  // Status badge color
  const statusColor = s => {
    const map = { "-": SLATE_400, "High": GREEN, "On Track": GREEN, "Excellent": GREEN, "Minimal": GREEN, "Monitor": AMBER, "Comprehensive Service": TEAL };
    return map[s] || SLATE_400;
  };
  const metricsRows = [
...
.brand-tagline { color: ${SLATE_500}; font-size: 7.5pt; margin-top: 2px; }
.report-title { color: ${GOLD}; font-size: 24pt; font-weight: 900; letter-spacing: 2px; }
/* ── Teal accent bar ──────────────── */
.accent-bar { background: ${TEAL}; height: 3px; }
/* ── Sub-header meta ──────────────── */
.meta-bar {
  background: ${NAVY2};
  padding: 9px 36px;
...
.meta-label { color: ${SLATE_600}; font-size: 6.5pt; text-transform: uppercase; letter-spacing: 0.07em; margin-bottom: 2px; }
.meta-value { color: white; font-size: 9.5pt; font-weight: 700; }
/* ── Page content ─────────────────── */
.page-content { padding: 22px 36px 56px; }
/* ── Section heading ──────────────── */
.section-title {
  font-size: 10.5pt; font-weight: 800; color: ${NAVY};
...
.metrics-table thead tr { background: ${NAVY}; }
...
.metrics-table tbody tr:nth-child(odd) td { background: ${SLATE_50}; }
...
.metric-cell { padding: 8px 12px; border: 0.5px solid ${SLATE_200}; color: ${SLATE_800}; width: 55%; line-height: 1.4; }
.value-cell { padding: 8px 12px; border: 0.5px solid ${SLATE_200}; color: ${SLATE_800}; font-weight: 700; width: 20%; text-align: center; }
.status-cell { padding: 8px 12px; border: 0.5px solid ${SLATE_200}; width: 25%; }
...
.cat-heading {
  font-weight: 700; font-size: 9pt; color: ${NAVY};
...
.col-half { flex: 1; padding: 12px 16px; background: ${SLATE_50}; border: 0.5px solid ${SLATE_200}; }
.col-half + .col-half { border-left: none; }
.col-heading { font-weight: 700; font-size: 9pt; color: ${NAVY}; margin-bottom: 7px; }
.col-item { font-size: 8.5pt; color: #374151; padding: 2px 0; }
.col-item::before { content: "• "; }
/* ── Time saved table ─────────────── */
.ts-note { font-size: 7.5pt; color: ${SLATE_400}; margin: 7px 0 10px; line-height: 1.55; font-style: italic; }
...
.data-table tbody tr:nth-child(even) td { background: ${SLATE_50}; }
.data-table tbody td { padding: 7px 12px; border: 0.5px solid ${SLATE_200}; color: #374151; }
.data-table tbody td:not(:first-child) { text-align: center; }
.data-table tfoot td {
  padding: 8px 12px; font-weight: 800; color: ${NAVY};
...
/* ── KPI Recap ────────────────────── */
.recap-meta { font-size: 8pt; color: ${SLATE_500}; margin-bottom: 14px; }
.recap-section { margin-bottom: 12px; }
.recap-label { font-weight: 800; font-size: 9.5pt; color: ${NAVY}; margin-bottom: 3px; }
...
.footer {
  ...
  background: ${NAVY}; border-top: 2px solid ${TEAL};
  padding: 9px 36px; display: flex; justify-content: space-between; align-items: center;
}
.footer-left { color: ${SLATE_600}; font-size: 7pt; }
.footer-right { color: ${SLATE_600}; font-size: 7pt; }
.footer-logo { color: ${TEAL}; font-weight: 800; font-size: 8pt; }
</style>
</head>
<body>
<!-- HEADER -->
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 3 }}>
        <span style={{ color: SLATE_400, maxWidth: "72%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{label}</span>
        <span style={{ color: SLATE_100, fontWeight: 700 }}>{value}h</span>
      </div>
...
const CAT_COLORS = {
  ...
  "Document Prep": ORANGE,
};
function catColor(c) { return CAT_COLORS[c] || SLATE_500; }
    ...
    <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", minHeight: "100vh" }}>
      <div style={{ background: "#060e1b", borderRight: "1px solid rgba(255,255,255,0.06)", overflowY: "auto", position: "sticky", top: 0, height: "100vh" }}>
        <div style={{ padding: "14px 12px 8px" }}>
          <div style={{ fontSize: 9, color: SLATE_600, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 8, display: "flex", alignItems: "center", gap: 5 }}>
            <span style={{ background: PURPLE, color: "#fff", width: 15, height: 15, borderRadius: "50%", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 8, fontWeight: 700 }}>1</span>
            Select VA
          </div>
          {allVAs.map(va => (
            <button key={va} onClick={() => setSelVA(va)}
              style={{ width: "100%", textAlign: "left", padding: "7px 9px", borderRadius: 6, marginBottom: 2, background: selVA === va ? "rgba(139,92,246,0.15)" : "transparent", border: selVA === va ? `1px solid ${PURPLE}` : "1px solid transparent", color: selVA === va ? "#fff" : SLATE_400, cursor: "pointer", fontSize: 12, fontWeight: selVA === va ? 600 : 400, display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ width: 26, height: 26, borderRadius: "50%", background: selVA === va ? PURPLE : "rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, color: selVA === va ? "#fff" : SLATE_600, flexShrink: 0 }}>{va.charAt(0).toUpperCase()}</div>
              {va}
            </button>
          ))}
        </div>
        ...
        <div style={{ padding: "10px 12px 16px" }}>
          <div style={{ fontSize: 9, color: SLATE_600, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 9, display: "flex", alignItems: "center", gap: 5 }}>
          ...
          {months.map(m => (
            <div key={m.month} style={{ marginBottom: 4 }}>
              <button onClick={() => setOpenMonth(openMonth === m.month ? null : m.month)}
                style={{ width: "100%", textAlign: "left", padding: "5px 9px", borderRadius: 6, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.05)", color: SLATE_500, cursor: "pointer", fontSize: 11, fontWeight: 600, display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 2 }}>
                <span>{m.month}</span><span style={{ fontSize: 9, color: SLATE_700 }}>{openMonth === m.month ? "▲" : "▼"}</span>
              </button>
              {openMonth === m.month && m.slots.map(p => {
                const active = selPeriod?.label === p.label;
                return (
                  <button key={p.label} onClick={() => setSelPeriod(p)}
                    style={{ width: "100%", textAlign: "left", padding: "6px 12px", borderRadius: 6, marginBottom: 2, background: active ? "rgba(240,180,41,0.1)" : "rgba(255,255,255,0.02)", border: active ? `1px solid ${GOLD}` : "1px solid transparent", color: active ? "#fff" : SLATE_400, cursor: "pointer", fontSize: 11 }}>
                    {p.label}
                  </button>
                );
              })}
      ...
      <div style={{ padding: "28px 36px", overflowY: "auto" }}>
        {!selVA && <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "70vh", opacity: 0.4, gap: 12 }}><div style={{ fontSize: 48 }}>👤</div><div style={{ fontSize: 16, fontWeight: 700, color: SLATE_400 }}>Select a VA + period</div></div>}
        {selVA && !selPeriod && <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "70vh", opacity: 0.5, gap: 10 }}><div style={{ fontSize: 36 }}>📅</div><div style={{ fontSize: 14, fontWeight: 700, color: SLATE_400 }}>Now select a reporting period</div></div>}
        {selVA && selPeriod && !data && <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "70vh", opacity: 0.5, gap: 10 }}><div style={{ fontSize: 36 }}>📭</div><div style={{ fontSize: 14, fontWeight: 700, color: SLATE_400 }}>No tasks found for {selVA} in this period</div></div>}
        {selVA && selPeriod && data && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {/* Header */}
            <div style={{ background: `linear-gradient(135deg,${NAVY2},#1a2744)`, borderRadius: 14, padding: "20px 26px", borderBottom: `3px solid ${PURPLE}` }}>
              <div style={{ fontSize: 9, color: PURPLE, textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 4 }}>VA Performance Dashboard</div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  <div style={{ width: 52, height: 52, borderRadius: "50%", background: `linear-gradient(135deg,${PURPLE},${PURPLE_DARK})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, fontWeight: 900, color: "#fff" }}>{selVA.charAt(0).toUpperCase()}</div>
                  <div>
                    <div style={{ fontSize: 22, fontWeight: 800, color: "#fff" }}>{selVA}</div>
                    <div style={{ fontSize: 10, color: SLATE_600, marginTop: 2 }}>Virtual Assistant · {selPeriod.label}</div>
                    <div style={{ display: "inline-block", marginTop: 5, padding: "2px 9px", borderRadius: 20, background: "rgba(139,92,246,0.2)", border: "1px solid rgba(139,92,246,0.4)", fontSize: 10, color: PURPLE_LIGHT, fontWeight: 700 }}>{rating}</div>
                  </div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10 }}>
                  {[["Total Hours", data.totalHours + "h", TEAL], ["Tasks Done", data.taskCount, GOLD], ["Clients Served", Object.keys(data.clientHours).length, PURPLE]].map(([l, v, a]) => (
                    <div key={l} style={{ background: "rgba(255,255,255,0.06)", borderRadius: 9, padding: "10px 14px", borderLeft: `3px solid ${a}`, minWidth: 90 }}>
                      <div style={{ fontSize: 8, color: SLATE_600, textTransform: "uppercase", marginBottom: 3 }}>{l}</div>
            ...
            {/* Insight banner */}
            <div style={{ background: "rgba(139,92,246,0.07)", border: "1px solid rgba(139,92,246,0.2)", borderRadius: 10, padding: "11px 16px", fontSize: 11, color: PURPLE_LIGHT, lineHeight: 1.7 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10 }}>
              {[["Output Rate", `${outputRate} tasks/hr`, "Higher = leaner delivery", TEAL], ["Category Breadth", `${breadth} types`, "Wider = more versatile", PURPLE], ["Hidden Effort Index", `+${hiddenHours}h`, "Estimated unpaid complexity", GOLD]].map(([l, v, note, a]) => (
                <div key={l} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: "13px 15px", borderLeft: `3px solid ${a}` }}>
                  <div style={{ fontSize: 9, color: SLATE_600, textTransform: "uppercase", marginBottom: 4 }}>{l}</div>
                  <div style={{ fontSize: 20, fontWeight: 800, color: "#fff", fontFamily: "monospace", marginBottom: 4 }}>{v}</div>
                  <div style={{ fontSize: 9, color: SLATE_600 }}>{note}</div>
                </div>
              ))}
            </div>
            {/* Work by category */}
                    ...
                    <div key={cat} style={{ background: "rgba(255,255,255,0.02)", borderRadius: 8, padding: "10px 13px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                          ...
                          <span style={{ fontSize: 12, fontWeight: 600, color: SLATE_100 }}>{cat}</span>
                        </div>
                        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                          <span style={{ fontSize: 10, color: SLATE_600 }}>logged: <strong style={{ color: SLATE_300 }}>{hrs}h</strong></span>
                          <span style={{ fontSize: 10, color: SLATE_600 }}>×{mult} =</span>
                          <span style={{ fontSize: 12, fontWeight: 700, color: GOLD }}>{realHrs}h real</span>
                        </div>
                      </div>
                      <div style={{ fontSize: 10, color: SLATE_600, fontStyle: "italic" }}>Includes: {CAT_HIDDEN_NOTE[cat] || "additional coordination"}</div>
                    </div>
                  );
                })}
              </div>
            ...
            {/* Hours by client */}
            <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "16px 18px" }}>
              <div style={{ fontSize: 10, color: PURPLE, textTransform: "uppercase", fontWeight: 700, marginBottom: 13 }}>Hours by Client</div>
                  ...
                  return (
                    <div key={client}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}>
                        <span style={{ color: SLATE_300, fontWeight: 500 }}>{client}</span>
                        <span style={{ color: SLATE_100, fontWeight: 700 }}>{hrs}h <span style={{ color: SLATE_600, fontWeight: 400 }}>({pct}%) · {tph} tasks/hr</span></span>
                      </div>
                      <div style={{ height: 5, background: "rgba(255,255,255,0.05)", borderRadius: 3 }}>
                        <div style={{ height: 5, borderRadius: 3, background: `linear-gradient(90deg,${PURPLE},${PURPLE_DARK})`, width: `${pct}%`, transition: "width 0.4s" }} />
                      </div>
                    </div>
                  );
                })}
              ...
              <div style={{ background: "rgba(240,180,41,0.05)", border: "1px solid rgba(240,180,41,0.15)", borderRadius: 12, padding: "16px 18px" }}>
                ...
                {data.wins.map((w, i) => (
                  <div key={i} style={{ padding: "9px 12px", background: "rgba(240,180,41,0.07)", border: "1px solid rgba(240,180,41,0.15)", borderRadius: 8, marginBottom: 6 }}>
                    <div style={{ fontSize: 10, color: SLATE_400, marginBottom: 3 }}>{w.client}</div>
                    <div style={{ fontSize: 12, color: AMBER_LIGHT, fontStyle: "italic" }}>"{w.note}"</div>
                  </div>
                ))}
              </div>
            )}
            {/* All tasks */}
            <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "16px 18px" }}>
              <div style={{ fontSize: 10, color: SLATE_100, textTransform: "uppercase", fontWeight: 700, marginBottom: 14 }}>All Tasks This Period</div>
              {Object.entries(data.tasks.reduce((acc, t) => { if (!acc[t.client]) acc[t.client] = []; acc[t.client].push(t); return acc; }, {})).map(([client, tasks]) => (
                <div key={client} style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: TEAL, marginBottom: 7, paddingBottom: 4, borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                    {client} <span style={{ color: SLATE_600, fontWeight: 400 }}>· {tasks.reduce((s, t) => s + t.hours, 0).toFixed(2)}h</span>
                  </div>
                    ...
                    {tasks.sort((a, b) => b.hours - a.hours).map((t, i) => (
                      <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "5px 9px", background: "rgba(255,255,255,0.02)", borderRadius: 7, gap: 8 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 7, minWidth: 0 }}>
                          ...
                          <span style={{ fontSize: 11, color: SLATE_300, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.name}</span>
                        </div>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                          <span style={{ fontSize: 9, color: SLATE_600 }}>{t.date}</span>
    ...
    <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, overflow: "hidden" }}>
      ...
      <div style={{ background: "rgba(255,255,255,0.05)", padding: "14px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 13, color: SLATE_100 }}>{va}</div>
            <div style={{ fontSize: 10, color: SLATE_600, marginTop: 1 }}>{detail.tasks.length} tasks · {detail.hours}h</div>
          </div>
        </div>
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: GOLD, fontFamily: "monospace" }}>{pct}%</div>
          <div style={{ fontSize: 9, color: SLATE_600 }}>of total hours</div>
        </div>
      </div>
      <div style={{ padding: "14px 16px" }}>
        {/* Hours bar */}
        <div style={{ marginBottom: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 9, color: SLATE_600, marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.06em" }}>
        ...
        {/* Category breakdown */}
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 9, color: SLATE_600, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 8 }}>By Category</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
            {Object.entries(detail.cats).sort((a,b) => b[1]-a[1]).map(([cat, hrs]) => (
              <div key={cat} style={{ display: "flex", alignItems: "center", gap: 5, padding: "3px 8px", borderRadius: 20, background: `${catColor(cat)}18`, border: `1px solid ${catColor(cat)}40` }}>
                ...
                <span style={{ fontSize: 10, color: SLATE_300 }}>{cat}</span>
        ...
        {/* Tasks */}
        <div>
          <div style={{ fontSize: 9, color: SLATE_600, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 7 }}>Tasks Completed</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
        ...
        {vaWins.length > 0 && (
          <div style={{ marginTop: 12 }}>
            <div style={{ fontSize: 9, color: SLATE_600, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 7 }}>Client Feedback</div>
            {vaWins.map((w, i) => (
              <div key={i} style={{ padding: "7px 10px", background: "rgba(240,180,41,0.07)", border: "1px solid rgba(240,180,41,0.2)", borderRadius: 7, fontSize: 11, color: AMBER_LIGHT, fontStyle: "italic" }}>
                ⭐ "{w.note}"
              </div>
            ))}
          </div>
...
function VAPerformance({ data }) {
  ...
  if (!data?.vaDetails) return (
    <div style={{ textAlign: "center", opacity: 0.4, padding: "48px 0" }}>
      ...
      <div style={{ fontSize: 13, color: SLATE_400, marginTop: 8 }}>No VA data for this period</div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, marginBottom: 16 }}>
        {[
          ["VAs Active", sorted.length, PURPLE],
          ...
          <div key={l} style={{ background: "rgba(255,255,255,0.04)", borderRadius: 9, padding: "11px 14px", borderLeft: `3px solid ${a}` }}>
            <div style={{ fontSize: 9, color: SLATE_600, textTransform: "uppercase", marginBottom: 3 }}>{l}</div>
            <div style={{ fontSize: 20, fontWeight: 800, color: SLATE_100, fontFamily: "monospace" }}>{v}</div>
          </div>
        ))}
      </div>
      {/* Sort controls */}
      <div style={{ display: "flex", gap: 6, marginBottom: 14, alignItems: "center" }}>
        <span style={{ fontSize: 10, color: SLATE_600, textTransform: "uppercase", letterSpacing: "0.06em" }}>Sort by:</span>
        {[["hours", "Hours"], ["tasks", "Tasks"], ["name", "Name"]].map(([k, label]) => (
          <button key={k} onClick={() => setSortBy(k)}
            style={{ padding: "4px 10px", borderRadius: 6, border: `1px solid ${sortBy === k ? TEAL : "rgba(255,255,255,0.08)"}`, background: sortBy === k ? "rgba(13,148,136,0.15)" : "transparent", color: sortBy === k ? TEAL : SLATE_500, cursor: "pointer", fontSize: 11, fontWeight: sortBy === k ? 700 : 400 }}>
            {label}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: "16px 14px 12px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 9, color: syncStatus === "ok" ? GREEN : syncStatus === "syncing" ? GOLD : SLATE_500 }}>
        ...
        {months.map(m => (
          <div key={m.month} style={{ marginBottom: 4 }}>
            <button onClick={() => setOpenMonth(openMonth === m.month ? null : m.month)}
              style={{ width: "100%", textAlign: "left", padding: "5px 9px", borderRadius: 6, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.05)", color: SLATE_500, cursor: "pointer", fontSize: 11, fontWeight: 600, display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 2 }}>
              <span>{m.month}</span>
              <span style={{ fontSize: 9, color: SLATE_700 }}>{openMonth === m.month ? "▲" : "▼"}</span>
            </button>
            {openMonth === m.month && m.slots.map(p => {
              const active = selPeriod?.label === p.label;
              return (
...
function MainPanel({ client, period, sheetData }) {
  ...
  if (!client && !period) return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "70vh", gap: 14, opacity: 0.4 }}>
      ...
      <div style={{ fontSize: 16, fontWeight: 700, color: SLATE_400 }}>Select a client + period</div>
      <div style={{ fontSize: 12, color: SLATE_600, textAlign: "center", lineHeight: 1.9 }}>Choose a client and reporting window from the sidebar to get started.</div>
    </div>
  );
  if (client && !period) return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "70vh", gap: 10, opacity: 0.5 }}>
      ...
      <div style={{ fontSize: 14, fontWeight: 700, color: SLATE_400 }}>Now select a reporting period</div>
    </div>
  );
  const roi = data ? {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ background: `linear-gradient(135deg,${NAVY2},#163456)`, borderRadius: 14, padding: "20px 26px", borderBottom: `3px solid ${GOLD}` }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8 }}>
          <div>
            <div style={{ fontSize: 10, color: SLATE_600, marginBottom: 1 }}>Prepared for</div>
            <div style={{ fontSize: 22, fontWeight: 800, color: "#fff" }}>{client.display}</div>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 10, color: SLATE_600, marginBottom: 1 }}>Reporting period</div>
            <div style={{ fontSize: 14, fontWeight: 700, color: "#fff" }}>{period.label}</div>
            <div style={{ fontSize: 10, color: SLATE_700, marginTop: 3 }}>{client.slack}</div>
          </div>
        </div>
      </div>
      {/* Data preview */}
        ...
        <>
          <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "16px 18px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, marginBottom: 14 }}>
              {[["Total Hours", `${data.totalHours}h`, TEAL], ["Tasks", data.taskCount, GOLD], ["VAs Active", Object.keys(data.vas).length, PURPLE]].map(([l, v, a]) => (
                <div key={l} style={{ background: "rgba(255,255,255,0.04)", borderRadius: 9, padding: "10px 12px", borderLeft: `3px solid ${a}` }}>
                  <div style={{ fontSize: 9, color: SLATE_600, textTransform: "uppercase", marginBottom: 3 }}>{l}</div>
                  <div style={{ fontSize: 19, fontWeight: 800, color: SLATE_100, fontFamily: "monospace" }}>{v}</div>
                </div>
              ))}
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div>
                <div style={{ fontSize: 9, color: SLATE_600, textTransform: "uppercase", marginBottom: 9 }}>Hours by Category</div>
              ...
              <div>
                <div style={{ fontSize: 9, color: SLATE_600, textTransform: "uppercase", marginBottom: 9 }}>VA Contribution</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
                {[["Opportunity Value", `$${roi.oppLow} – $${roi.oppHigh}`, GOLD], ["Cost Avoidance", `$${roi.avoid}`, TEAL], ["Self-Hours Saved", `${Math.round(data.totalHours * 1.5 * 10) / 10}h`, GREEN]].map(([l, v, a]) => (
                  <div key={l} style={{ background: "rgba(255,255,255,0.04)", borderRadius: 9, padding: "12px 13px", borderLeft: `3px solid ${a}` }}>
                    <div style={{ fontSize: 9, color: SLATE_600, marginBottom: 4, textTransform: "uppercase" }}>{l}</div>
                    <div style={{ fontSize: 13, fontWeight: 800, color: SLATE_100, fontFamily: "monospace" }}>{v}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {!data && status === "idle" && (
        <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "24px", textAlign: "center" }}>
          ...
          <div style={{ fontSize: 13, color: SLATE_500, lineHeight: 1.8 }}>No sheet data for this period. Claude will generate from available context.</div>
        </div>
      )}
      {/* Loading */}
      {status === "loading" && (
        <div style={{ background: "rgba(13,148,136,0.07)", border: `1px solid rgba(13,148,136,0.2)`, borderRadius: 14, padding: "36px", textAlign: "center" }}>
          ...
          <div style={{ fontSize: 11, color: SLATE_600 }}>Claude is compiling tasks, metrics, and narrative…</div>
        ...
        <div style={{ background: "rgba(13,148,136,0.07)", border: `1px solid rgba(13,148,136,0.25)`, borderRadius: 14, overflow: "hidden" }}>
          <div style={{ background: "rgba(13,148,136,0.12)", borderBottom: "1px solid rgba(13,148,136,0.2)", padding: "14px 18px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              ...
              <div style={{ fontSize: 10, color: SLATE_600, marginTop: 1 }}>{client.display} · {period.label} · {reportData.totals?.total_hours}h</div>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => { setStatus("idle"); setReportData(null); }}
                style={{ padding: "7px 14px", borderRadius: 7, border: "1px solid rgba(255,255,255,0.1)", background: "transparent", color: SLATE_500, cursor: "pointer", fontSize: 11, fontWeight: 600 }}>
                ↩ Regenerate
              </button>
              <button onClick={printAsPDF}
                style={{ padding: "9px 18px", borderRadius: 8, border: "none", background: `linear-gradient(135deg,${GOLD},${AMBER_DARK})`, color: NAVY, cursor: "pointer", fontSize: 12, fontWeight: 800, display: "flex", alignItems: "center", gap: 6, boxShadow: "0 3px 14px rgba(240,180,41,0.35)" }}>
                🖨️ Save as PDF
              </button>
              <button onClick={downloadHTML}
                style={{ padding: "9px 18px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.15)", background: "rgba(255,255,255,0.06)", color: SLATE_200, cursor: "pointer", fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}>
                ⬇ HTML
              </button>
            </div>
          </div>
          ...
          <div style={{ padding: "0 20px 16px", fontSize: 11, color: SLATE_500 }}>
            ...
            <span style={{ color: SLATE_500 }}>⬇ HTML saves the raw file for editing.</span>
          </div>
        </div>
      )}
      {/* Error */}
      {status === "error" && (
        <div style={{ background: "rgba(239,68,68,0.07)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 12, padding: "20px" }}>
          <div style={{ fontSize: 13, color: RED_400, marginBottom: 8, fontWeight: 700 }}>⚠️ Generation failed</div>
          {errorMsg && <div style={{ fontSize: 10, color: RED_300, marginBottom: 12, fontFamily: "monospace", background: "rgba(0,0,0,0.2)", padding: "8px 10px", borderRadius: 6, wordBreak: "break-all", lineHeight: 1.6 }}>{errorMsg}</div>}
          <button onClick={() => { setStatus("idle"); setErrorMsg(""); }} style={{ padding: "7px 16px", borderRadius: 7, border: "1px solid rgba(239,68,68,0.3)", background: "transparent", color: RED_400, cursor: "pointer", fontSize: 11 }}>Try Again</button>
        </div>
      )}
      {/* Generate button */}
      {status === "idle" && (
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: NAVY, fontFamily: "'DM Sans','Inter',sans-serif", color: SLATE_100, display: "flex", flexDirection: "column" }}>
      {/* Top nav */}
      <div style={{ background: "#040c18", borderBottom: "1px solid rgba(255,255,255,0.06)", padding: "0 24px", display: "flex", alignItems: "center", gap: 0, height: 46, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginRight: 28 }}>
          ...
          <span style={{ fontSize: 9, color: syncStatus === "ok" ? GREEN : syncStatus === "csv" ? GOLD : syncStatus === "syncing" ? GOLD : SLATE_600, marginLeft: 4 }}>
          ...
          <button onClick={fetchLiveData} disabled={syncStatus === "syncing"}
            ...
            style={{ marginLeft: 6, padding: "2px 7px", borderRadius: 5, border: "1px solid rgba(255,255,255,0.1)", background: "transparent", color: syncStatus === "syncing" ? SLATE_700 : SLATE_500, cursor: syncStatus === "syncing" ? "default" : "pointer", fontSize: 11, lineHeight: 1 }}>
            {syncStatus === "syncing" ? "⏳" : "🔄"}
          </button>
        ...
        {[["client", "📋 Client KPIs"], ["va", "👤 VA Dashboard"]].map(([tab, label]) => (
          <button key={tab} onClick={() => setAppTab(tab)}
            style={{ padding: "0 18px", height: "100%", border: "none", borderBottom: appTab === tab ? `2px solid ${TEAL}` : "2px solid transparent", background: "transparent", color: appTab === tab ? "#fff" : SLATE_600, cursor: "pointer", fontSize: 12, fontWeight: appTab === tab ? 700 : 400, transition: "all 0.15s" }}>
            {label}
          </button>
        ))}
      </div>
